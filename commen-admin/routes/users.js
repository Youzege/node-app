...
router.post('/login', async (ctx) => {
  try {
     ...
     * 2. json  { userId: 1, _id: 0 }  1返回 0不返回
     * 3.    .select('userId')
     */
...
router.post('/operate', async (ctx) => {
  ...
  if (action == 'add') {
    ...
    const res = await User.findOneAndUpdate({ $or: [{ userName }, { userEmail }] }, '_id userName userEmail')
    if (res) {
...
async function getMenuList(userRole, roleKeys) {
  ...
  if (userRole == 0) {
    ...
    // 根据用户拥有的角色，获取权限列表
    // 先查找用户对应的角色有哪些 $in相当于包含、等于，查询时查找包含于指定字段条件的数据
